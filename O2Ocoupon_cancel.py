...
func_list=[Cid,dr,Label,output_label,Action]        
#off 需要cid,dr,label,output_label,action
#online 需要cid,dr,label,output_label
#test 因为是线下 需要cid,dr,action
#很忧伤，竟然用不了label，
def tidy_data(df,f):
...
def prepare(dataset,func_list):
    ...
    return dataset
ndataset=prepare(dataset,func_list)

df_off=ndataset[0].drop(columns=['Coupon_id','Discount_rate','Distance','Date','interval','Date_received'])
df_on=ndataset[1].drop(columns=['Coupon_id','Discount_rate','Distance','Date','interval','Date_received'])
df_test=ndataset[2].drop(columns=['Coupon_id', 'Discount_rate', 'Distance','Date_received'])
