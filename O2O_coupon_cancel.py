...
import math
import time
ccf_offline_test=pd.read_csv(r'C:\Users\Frank_li\python_script\O2Ocoupon_cancel\ccf_offline_stage1_test_revised.csv')
coupon_sample = pd.read_csv(r'C:\Users\Frank_li\python_script\O2Ocoupon_cancel\sample_submission.csv',header=0)
ccf_offline_train=pd.read_csv(r'C:\Users\Frank_li\python_script\O2Ocoupon_cancel\ccf_offline_stage1_train.csv')
"""
...
"""
ccf_online_train=pd.read_csv(r'C:\Users\Frank_li\python_script\O2Ocoupon_cancel\ccf_online_stage1_train.csv')
...
设定标签。作为标记向量。
ccf_offline_train['Label']=ccf_offline_train.apply(lambda x:alter_label(x),axis=1)
...
对应的Label 则只有-1和0两种情形。-1表示领取未使用。0表示未领取。
...
def alter_distance(x):
    if pd.isna(x['Distance']):
        return -1
    else:
        return x['Distance']
ccf_offline_train['distance']=ccf_offline_train.apply(alter_distance,axis=1)
        
2.discount_rate 需要处理,我认为满300减60不等于优惠20%，也不等于满100减20，这个和买的是什么商品有关。
3.Date_received Date_received领取优惠券日期 这个与Date构成了Interval，而且补全他的NA值意义不大，一是其与总数据少了
2个数量级，二是没有好的填充方式。
...
def alter_id(x):
    if pd.isna(x['Coupon_id']):
        ...
        return 1
ccf_offline_train['New_Cid']=ccf_offline_train.apply(alter_id,axis=1)
ccf_online_train['New_Cid']=ccf_online_train.apply(alter_id,axis=1)
...
6.Merchant_id和user_id的作用可以是类似于index，这类无特别含义的信息，不放入模型中。groupby这个范围还是太大了。
